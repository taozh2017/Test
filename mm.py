...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep  2 15:04:12 2019
...
class conv_multi_style_fusion_block(nn.Module):
    def __init__(self,in_dim, out_dim,act_fn):
        super(conv_multi_style_fusion_block, self).__init__()
...
class conv_multi_style_fusion_block0(nn.Module):
    def __init__(self,in_dim, out_dim,act_fn):
        super(conv_multi_style_fusion_block0, self).__init__()
...
class conv_multi_style_fusion_block_TRIPLE0(nn.Module):
    def __init__(self,in_dim, out_dim,act_fn):
        super(conv_multi_style_fusion_block_TRIPLE0, self).__init__()
    ...
    def forward(self, x1,x2,x3):
        ...
        # multi-style fusion
        fusion_sum = torch.add(torch.add(x1, x2),x3)   # sum
        fusion_mul = torch.mul(torch.mul(x1, x2),x3)
         
        modal_in1  = torch.reshape(x1,[x1.shape[0],1,x1.shape[1],x1.shape[2],x1.shape[3]])
        modal_in2  = torch.reshape(x2,[x2.shape[0],1,x2.shape[1],x2.shape[2],x2.shape[3]])
        modal_in3  = torch.reshape(x3,[x3.shape[0],1,x3.shape[1],x3.shape[2],x3.shape[3]])
        modal_cat  = torch.cat((modal_in1, modal_in2,modal_in3),dim=1)
        fusion_max = modal_cat.max(dim=1)[0]
...
class conv_multi_style_fusion_block_TRIPLE(nn.Module):
    def __init__(self,in_dim, out_dim,act_fn):
        super(conv_multi_style_fusion_block_TRIPLE, self).__init__()
...
class conv_multi_style_fusion_block_simplefusion(nn.Module):
    def __init__(self,in_dim, out_dim,act_fn):
        super(conv_multi_style_fusion_block_simplefusion, self).__init__()
        
        self.layer1 = nn.Sequential(nn.Conv2d(in_dim*2, in_dim, kernel_size=3, stride=1, padding=1),nn.BatchNorm2d(in_dim),act_fn,)
    ...
    def forward(self, x1,x2,xx):
        ...
        out_fusion = torch.cat((x1,x2),dim=1)
...
class conv_multi_style_fusion_block_simplefusion0(nn.Module):
    def __init__(self,in_dim, out_dim,act_fn):
        super(conv_multi_style_fusion_block_simplefusion0, self).__init__()
...
###############################################################################
###############################################################################
#revised in 08/18/2019.
#without using attention

class IVD_Net_asym_no_atten_autoencoder(nn.Module):

    def __init__(self,input_nc, output_nc, ngf):
        super(IVD_Net_asym_no_atten_autoencoder,self).__init__()
        print('~'*50)
        print(' ----- Creating FUSION_NET HD (Assymetric) network 09/03/2019...')
        print('~'*50)
        ...
        #######################################################################
        # down 1st layer
        self.down_fu_1 = conv_multi_style_fusion_block0(self.out_dim,self.out_dim*2,act_fn)
        self.pool_fu_1 = maxpool()
        
        self.down_fu_2 = conv_multi_style_fusion_block(self.out_dim*2,self.out_dim*4,act_fn)
        self.pool_fu_2 = maxpool()
        
        self.down_fu_3 = conv_multi_style_fusion_block(self.out_dim*4,self.out_dim*4,act_fn)
        ...
        self.deconv_2_0 = conv_multi_style_fusion_block(self.out_dim * 4, self.out_dim * 2,act_fn2)
        self.deconv_3_0 = conv_multi_style_fusion_block(self.out_dim * 2, self.out_dim * 1,act_fn2)
        self.deconv_4_0 = conv_multi_style_fusion_block(self.out_dim * 1, self.out_dim,act_fn2)  
...
class IVD_Net_asym_no_atten_autoencoder_FusionSimple(nn.Module):

    def __init__(self,input_nc, output_nc, ngf):
        super(IVD_Net_asym_no_atten_autoencoder_FusionSimple,self).__init__()
        print('~'*50)
        print(' ----- Creating FUSION_NET HD (Assymetric) network 09/03/2019...')
        print('~'*50)
        ...
        #######################################################################
        # down 1st layer
        self.down_fu_1 = conv_multi_style_fusion_block_simplefusion0(self.out_dim,self.out_dim*2,act_fn)
        self.pool_fu_1 = maxpool()
        
        self.down_fu_2 = conv_multi_style_fusion_block_simplefusion(self.out_dim*2,self.out_dim*4,act_fn)
        self.pool_fu_2 = maxpool()
        
        self.down_fu_3 = conv_multi_style_fusion_block_simplefusion(self.out_dim*4,self.out_dim*4,act_fn)
        self.pool_fu_3 = maxpool()
...
class IVD_Net_asym_no_atten_autoencoder_GenerateSimple(nn.Module):

    def __init__(self,input_nc, output_nc, ngf):
        super(IVD_Net_asym_no_atten_autoencoder_GenerateSimple,self).__init__()
        print('~'*50)
        print(' ----- Creating FUSION_NET HD (Assymetric) network 09/03/2019...')
        print('~'*50)
        ...
        # ~~~ Decoding Path ~~~~~~ #
        self.deconv_1_0 = conv_decod_block(self.out_dim * 8, self.out_dim * 4, act_fn2)
        self.deconv_2_0 = conv_multi_style_fusion_block_simplefusion(self.out_dim * 4, self.out_dim * 2,act_fn2)
        self.deconv_3_0 = conv_multi_style_fusion_block_simplefusion(self.out_dim * 2, self.out_dim * 1,act_fn2)
        self.deconv_4_0 = conv_multi_style_fusion_block_simplefusion(self.out_dim * 1, self.out_dim,act_fn2)  
        self.deconv_5_0 = conv_decod_block(self.out_dim * 1, self.out_dim,act_fn2) 
        self.out        = nn.Sequential(nn.Conv2d(int(self.out_dim),1, kernel_size=3, stride=1, padding=1),nn.Tanh()) #  self.final_out_dim
...
class IVD_Net_asym_no_atten_autoencoder_Simple(nn.Module):

    def __init__(self,input_nc, output_nc, ngf):
        super(IVD_Net_asym_no_atten_autoencoder_Simple,self).__init__()
        print('~'*50)
        print(' ----- Creating FUSION_NET HD (Assymetric) network 09/03/2019...')
        print('~'*50)
...
class IVD_Net_asym_no_atten_autoencoder_TRIPLE(nn.Module):

    def __init__(self,input_nc, output_nc, ngf):
        super(IVD_Net_asym_no_atten_autoencoder_TRIPLE,self).__init__()
        print('~'*50)
        print(' ----- Creating FUSION_NET HD (Assymetric) network 09/03/2019...')
        print('~'*50)
        ...
        self.down_2_2 = nn.Sequential(
                nn.Conv2d(in_channels=self.out_dim,  out_channels=self.out_dim*2, kernel_size=3, stride=1, padding=1),nn.BatchNorm2d(self.out_dim*2), act_fn,
                nn.Conv2d(in_channels=self.out_dim*2,out_channels=self.out_dim*2, kernel_size=3, stride=1, padding=1),nn.BatchNorm2d(self.out_dim*2), act_fn,
                )
        self.pool_2_2 = maxpool()
        
        self.down_3_2 = nn.Sequential(
                nn.Conv2d(in_channels=self.out_dim*2,  out_channels=self.out_dim*4, kernel_size=3, stride=1, padding=1),nn.BatchNorm2d(self.out_dim*4), act_fn,
                nn.Conv2d(in_channels=self.out_dim*4,out_channels=self.out_dim*4, kernel_size=3, stride=1, padding=1),nn.BatchNorm2d(self.out_dim*4), act_fn,
                )
        ...
        #######################################################################
        # down 1st layer
        self.down_fu_1 = conv_multi_style_fusion_block_TRIPLE0(self.out_dim,self.out_dim*2,act_fn)
        self.pool_fu_1 = maxpool()
        
        self.down_fu_2 = conv_multi_style_fusion_block_TRIPLE(self.out_dim*2,self.out_dim*4,act_fn)
        self.pool_fu_2 = maxpool()
        
        self.down_fu_3 = conv_multi_style_fusion_block_TRIPLE(self.out_dim*4,self.out_dim*4,act_fn)
        self.pool_fu_3 = maxpool()
        ...
        # ~~~ Decoding Path ~~~~~~ #
        self.deconv_1_0 = conv_decod_block(self.out_dim * 8, self.out_dim * 4, act_fn2)
        self.deconv_2_0 = conv_multi_style_fusion_block_TRIPLE(self.out_dim * 4, self.out_dim * 2,act_fn2)
        self.deconv_3_0 = conv_multi_style_fusion_block_TRIPLE(self.out_dim * 2, self.out_dim * 1,act_fn2)
        self.deconv_4_0 = conv_multi_style_fusion_block_TRIPLE(self.out_dim * 1, self.out_dim,act_fn2)  
        self.deconv_5_0 = conv_decod_block(self.out_dim * 1, self.out_dim,act_fn2) 
        self.out        = nn.Sequential(nn.Conv2d(int(self.out_dim),1, kernel_size=3, stride=1, padding=1),nn.Tanh()) #  self.final_out_dim
        ...
        # modality 3
        self.deconv_1_3 = conv_decod_block(self.out_dim * 4, self.out_dim * 4, act_fn2)
        self.deconv_2_3 = conv_decod_block(self.out_dim * 4, self.out_dim * 4, act_fn2)        
        self.deconv_3_3 = conv_decod_block(self.out_dim * 4, self.out_dim * 2, act_fn2)
        self.deconv_4_3 = conv_decod_block(self.out_dim * 2, self.out_dim * 2, act_fn2)
        self.deconv_5_3 = conv_decod_block(self.out_dim * 2, self.out_dim * 1, act_fn2)
        self.deconv_6_3 = conv_decod_block(self.out_dim * 1, int(self.out_dim), act_fn2)
        self.out3       = nn.Sequential(nn.Conv2d(int(self.out_dim),1, kernel_size=3, stride=1, padding=1),nn.Tanh()) #  self.final_out_dim
    ...
    def forward(self,inputs):
        ...
        down_1_0m   = self.pool_1_0(down_1_0)
        down_1_1m   = self.pool_1_1(down_1_1)
        down_1_2m   = self.pool_1_2(down_1_2)
        
        down_2_0 = self.down_2_0(down_1_0m)
        down_2_1 = self.down_2_1(down_1_1m)
        down_2_2 = self.down_2_2(down_1_2m)
        ...
        # ----------------------------------------
        # fusion layer
        down_fu_1   = self.down_fu_1(down_1_0m,down_1_1m,down_1_2m)                                                                                                         
        down_fu_1m  = self.pool_fu_1(down_fu_1)
        
        down_fu_2   = self.down_fu_2(down_fu_1m,down_2_0m,down_2_1m,down_2_2m)                                                                                                         
        down_fu_2m  = self.pool_fu_2(down_fu_2)
        
        down_fu_3   = self.down_fu_3(down_fu_2m,down_3_0m,down_3_1m,down_3_2m) 
        down_fu_4   = self.down_fu_4(down_fu_3)
        ...
        # ~~~~~~ Decoding 
        deconv_1_0 = self.deconv_1_0(down_fu_4)
        deconv_2_0 = self.deconv_2_0(down_3_0m,down_3_1m,down_3_2m,deconv_1_0)
        deconv_3_0 = self.deconv_3_0(down_2_0m,down_2_1m,down_2_2m,up(deconv_2_0))
        deconv_4_0 = self.deconv_4_0(down_1_0m,down_1_1m,down_1_2m,up(deconv_3_0))
        deconv_5_0 = self.deconv_5_0(up(deconv_4_0))
        output     = self.out(deconv_5_0)
        ...
        # modality 3
        deconv_1_3 = self.deconv_1_3((down_3_2m))
        deconv_2_3 = self.deconv_2_3(up(deconv_1_3))
        deconv_3_3 = self.deconv_3_3((deconv_2_3))
        deconv_4_3 = self.deconv_4_3(up(deconv_3_3))
        deconv_5_3 = self.deconv_5_3((deconv_4_3))
        deconv_6_3 = self.deconv_6_3(up(deconv_5_3))
        output3    = self.out(deconv_6_3)   
